...
def sample_gaussian(m, v):
    ...
    #sample e from a normal gaussian
    # z = m + v*e
    # epsilon = torch.randn_like(m)
    # z = m + torch.exp(0.5 * v) * epsilon
...
def log_normal(x, m, v):
    ...
    # End of code modification
    ################################################################################
    normal = dist.Normal(m,torch.sqrt(v))
    ...
    # log_v = torch.log(torch.sqrt(v))

    # log_prob = -log_v - 0.5*(np.log((2*np.pi))) - ((x-m)**2)/(2*v)
...
def log_normal_mixture(z, m, v):
    ...
    # End of code modification
    ################################################################################
    mix_z = z.unsqueeze(1).expand_as(m)
    # log_v = torch.log(torch.sqrt(v))
    # log_prob = -log_v - 0.5*(np.log((2*np.pi))) - ((mix_z-m)**2)/(2*v)


    # mixture = log_prob.sum(-1)
    # log_prob = log_mean_exp(mixture, -1)
    # return log_prob

    normal = dist.Normal(m,torch.sqrt(v))
    log_prob = normal.log_prob(mix_z)
    log_prob = log_prob.sum(-1)
...
def mse_loss(x, logits):
    ...
    Return:
        log_prob: tensor: (batch,): log probability of each sample
    """
    log_prob = -mse(input=logits, target=x).sum(-1)
    return log_prob
def log_cosh_loss(x, logits, a = 100):
    # Log-cosh loss
    error = a*(x-logits)
    #loss = -torch.sum(torch.log(torch.cosh(error + 1e-12)))  # Adding epsilon to avoid numerical instability
    loss = -torch.sum(error + F.softplus(-2. * error) - math.log(2.0))
    loss = loss/a
    return loss
...
def convert_dataset_toTensor(dataset_path):
    ...
    custom_dataset = datasets.ImageFolder(root=dataset_path, transform=preprocess)
    return custom_dataset
def load_custom_dataset(dataset, device,pixels, batch_size=16, shuffle=True):
    #test multiple image resolutions. Start with 64- 256
    ...
    # Extract data and labels from the loader
    data = next(iter(custom_loader))
    images, labels = data
    for i in range(batch_size):
        # Convert the tensor to a NumPy array and reshape it to (pixels, pixels, 3)
        image_np = np.transpose(images[i].cpu().numpy().reshape((3, pixels, pixels)), (1, 2, 0))
    ...
    total_rgb = pixels*pixels*3
    print("total rbg: ",total_rgb)
    
    images = images.to(device).view(-1, total_rgb).float() / 255
   

    # Convert labels to the desired device
    labels = labels.to(device)
    

    return custom_loader, labels
